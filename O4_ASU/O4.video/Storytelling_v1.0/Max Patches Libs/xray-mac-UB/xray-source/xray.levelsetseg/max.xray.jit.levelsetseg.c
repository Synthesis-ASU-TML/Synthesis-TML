...
#define LSS_LAST_MATRIX	0
#define LSS_LAST_VAL		1

typedef struct _max_xray_jit_levelsetseg 
{
	t_object		ob;
	void			*obex;
	float			val;
...
} t_max_xray_jit_levelsetseg;

t_jit_err xray_jit_levelsetseg_init(void); 

void *max_xray_jit_levelsetseg_new(t_symbol *s, long argc, t_atom *argv);
void max_xray_jit_levelsetseg_free(t_max_xray_jit_levelsetseg *x);
void *max_xray_jit_levelsetseg_class;
...
{	
	...
	setup(&max_xray_jit_levelsetseg_class, max_xray_jit_levelsetseg_new, (method)max_xray_jit_levelsetseg_free, (short)sizeof(t_max_xray_jit_levelsetseg), 
		0L, A_GIMME, 0);
...
void *max_xray_jit_levelsetseg_new(t_symbol *s, long argc, t_atom *argv)
{
	t_max_xray_jit_levelsetseg *x;
	void *o;

	if (x=(t_max_xray_jit_levelsetseg *)max_jit_obex_new(max_xray_jit_levelsetseg_class,gensym("xray_jit_levelsetseg"))) {
		if (o=jit_object_new(gensym("xray_jit_levelsetseg"))) {
			max_jit_mop_setup_simple(x,o,argc,argv);			
			max_jit_attr_args(x,argc,argv);
		} else {
			error("xray.jit.levelsetseg: could not allocate object");
			freeobject(x);
		}
	}
	return (x);
