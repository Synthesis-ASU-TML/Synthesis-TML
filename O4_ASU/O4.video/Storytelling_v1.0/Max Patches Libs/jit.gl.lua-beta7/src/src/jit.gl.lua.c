...
t_jit_err jit_gl_lua_init(void)
{
	...
	jit_class_addmethod(_jit_gl_lua_class,
						(method)jit_gl_lua_call,			"call", A_DEFER_LOW, 0L);
	jit_class_addmethod(_jit_gl_lua_class,
						(method)jit_gl_lua_call_list,		"call_list", A_DEFER_LOW, 0L);
	jit_class_addmethod(_jit_gl_lua_class,
	...
	// file
	attr = jit_object_new(_jit_sym_jit_attr_offset,"file",_jit_sym_symbol,attrflags,
		(method)0L,(method)jit_gl_lua_setfile,calcoffset(t_jit_gl_lua,file));
	jit_class_addattr(_jit_gl_lua_class, attr);

	// autogarbage
	attr = jit_object_new(_jit_sym_jit_attr_offset,"autogarbage",_jit_sym_long,attrflags,
		(method)0L,(method)0L,calcoffset(t_jit_gl_lua,autogarbage));
	jit_class_addattr(_jit_gl_lua_class, attr);
	
	// gc
	attr = jit_object_new(_jit_sym_jit_attr_offset,"gc",_jit_sym_long,attrflags,
	...
	// memsize
	attr = jit_object_new(_jit_sym_jit_attr_offset,"memsize",_jit_sym_long,attrflags,
		(method)jit_gl_lua_getmemsize,(method)0L,calcoffset(t_jit_gl_lua,memsize));
	jit_class_addattr(_jit_gl_lua_class, attr);
	
	// context
	attr = jit_object_new(_jit_sym_jit_attr_offset,"context",_jit_sym_long,attrflags,
	...
	// inlets
	attr = jit_object_new(_jit_sym_jit_attr_offset,"inlets",_jit_sym_long,attrflags,
		(method)0L,(method)0L,calcoffset(t_jit_gl_lua,inlets));
	jit_class_addattr(_jit_gl_lua_class, attr);
	
	// outlets
	attr = jit_object_new(_jit_sym_jit_attr_offset,"outlets",_jit_sym_long,attrflags,
		(method)0L,(method)0L,calcoffset(t_jit_gl_lua,outlets));
	jit_class_addattr(_jit_gl_lua_class, attr);
	
	// lua_unique_name
	attr = jit_object_new(_jit_sym_jit_attr_offset,"lua_unique_name",_jit_sym_symbol,attrflags,
...
t_jit_err jit_gl_lua_getmemsize(t_jit_gl_lua *x, void *attr, long *argc, t_atom **argv)
{
	if ((*argc)&&(*argv)) {
		...
		*argc = 1;
		if (!(*argv = jit_getbytes(sizeof(t_atom)*(*argc)))) {
			*argc = 0;
...
t_jit_err jit_gl_lua_call_luafunction(t_jit_gl_lua *x, const char *function)
{
	...
	if(x->texthandle) {
		...
		if(lua_isfunction(x->lua, -1)) {
			...
			//have to pop value (which is most likely nil), else will eventuall get a stack overflow
			lua_pop(x->lua, -1);
		}
	}
