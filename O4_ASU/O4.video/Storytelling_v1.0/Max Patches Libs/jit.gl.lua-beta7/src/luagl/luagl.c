...
static const char *get_str_gl_enum(lua_State *L, GLenum num)
{
	const char *val;
...
/*PointParameter (params) -> none*/
static int gl_PointParameter(lua_State *L)
{
	if(lua_type(L, 1) != LUA_TSTRING) {
		luaL_error(L, "incorrect argument 1 to function 'gl.PointParameter'");
	}

	/* test arguments type */
	if(lua_isnumber(L, 2)) {
		glPointParameterf(get_gl_enum(L, 1), (GLfloat)lua_tonumber(L, 2));
	}
	else if(lua_istable(L, 2)) {
		float *vals;
		int n;

		n = get_arrayf(L, 2, &vals);
		glPointParameterfv(get_gl_enum(L, 1), vals);

		free(vals);
	}
	else {
		luaL_error(L, "incorrect argument 2 to function 'gl.PointParameter'");
	}
...
static int gl_test_enum(lua_State *L)
{
	...
	if(lua_type(L, 1) != LUA_TSTRING) {
		luaL_error(L, "incorrect argument to function gl.test_enum");
	}
...
static int gl_test_string(lua_State *L)
{
	...
	if(! lua_isnumber(L, 1)) {
		luaL_error(L, "incorrect argument to function gl.test_string");
	}
