...
if (max.version < 455)
	npejies_error(this, "MaxMSP 4.5.5 or higher is required. Please upgrade!");
...
function anything()
{
	...
	if (tmp == -1) {	// si -1 c'est que ça ne correspond pas à un nom de function ou de message
		npejies_error(this, "doesn't understand", messagename, "(bad function name or message name)");
		return;
	}
...
function onresize(w,h)
{
	...
	if (sketch.fsaa)
		box.size(np_clip(w, 100, 1024),np_clip(h, 50, 1024));
	else
		box.size(np_clip(w, 100, 2048),np_clip(h, 50, 2048));
...
function LectureInspector()
{
	...
	if (jsarguments.length > 2) {
		...
		if (jsarguments.length != (11 + NbCourbes*18) ) {
			npejies_error(this, "bad number of arguments in the inspector");
			return;
		}
...
function PattrInterpError(v)
{
	...
	if (v == 0 )
		npejies_error(this, "interpolation isn't possible (different number of function)");
	else
		npejies_error(this, "interpolation isn't possible (different number of points)");
...
function PattrTooMany(v)
{
	if (v >= 2048) {
		if (PattrTooMany.flag) {
			npejies_error(this, "too many functions/points... pattr won't work...");
			PattrTooMany.flag = 0;
		}
	} else
		PattrTooMany.flag = 1;
...
{
	if (start >= stop) {
		npejies_error(this, "bad values for message domain: min must be lower than max");
		return;
	}
...
function MyPasteColors(courbe)
{
	...
	if (cp.length == 0) {
		npejies_error(this, "before pasting colors, you have to copy something");
		return;
	}
...
function MovePoint(courbe, lequel, newx, newy, curve)
{
	...
	if (lequel >= courbe.np) {
		npejies_error(this, "no point", lequel,"in function", courbe.name);
		return;
	}
	...
	if(MoveMode == 1) {
		...
		if(dx != 0) {
			...
			for(var i = lequel; i < courbe.np; i++) {
				courbe.pa[i].valx = np_clip(dx+courbe.pa[i].valx, courbe.domain[0], courbe.domain[1]);
				courbe.pa[i].x = val2x(courbe, courbe.pa[i].valx);
			}
		}
...
/* 	var NeedDraw = 0;	// pour savoir si on doit réafficher */
	...
	// en fonction du nombre d'arguments 1 (interpolationX-Y) 2 (AddPoint) 3 (MovePoint)
		switch (a.length) {
			...
			default: npejies_error(this, "too many arguments for message", msg); break;
		}
	...
	switch (a[0]) {
		...
		case "domain":
							if (a.length == 2)
								...
								npejies_error(this, "bad argument(s) for message domain"); 
							NeedNotify++; break;
		case "setdomain":	
							if (a.length == 2)
								...
								npejies_error(this, "bad argument(s) for message setdomain");
							NeedNotify++; break;
		...
		default: 			// message d'erreurs en fonction du type de message all ou pas
							if (msg == "all") {
								if (DisplayOneTime == 1) {
									npejies_error(this, "bad arguments for message all");
									DisplayOneTime = 0;
								}
							} else {
								npejies_error(this, "doesn't understand", a[0]);
								return 0;
							}
							break;
	}
...
function FixPoint(courbe, WhichPoint, state)
{
	if ( (state == 0 || state == 1 ) && (WhichPoint >= 0 && WhichPoint < courbe.np) ) {
		...
		npejies_error(this, "bad argument for message fix");
}
FixPoint.local = 1;
...
function MySustain(point, state, courbe)
{
	if ( state == 1 || state == 0 ) {
		if (point >= 0 && point < courbe.np) {
			...
			npejies_error(this, "no point", point, "(sustain operation aborted)" );
	} else
		npejies_error(this, "bad arguments for message sustain");
}
MySustain.local = 1;
...
function MyGridStep_x(courbe, v)
{
	if (typeof(v) == "number" && v > 0) {
		...
		npejies_error(this, "bad argument for message gridstep_x");
}
...
function MyGridStep_y(courbe, v)
{
	if (typeof(v) == "number" && v > 0) {
		...
		npejies_error(this, "bad argument for message gridstep_y");
}
...
function checkInputMatrix(input)
{
	...
	if (input.dim.length < 2 || input.dim.length > 3) {
		...
		post();
		npejies_error(this, "support only 2/3 dim matrix");
		isOk = false;
	}
	if (input.planecount.length > 1) {
		npejies_error(this, "support only 1 plane matrix");
		isOk = false;
	}
	return isOk;
}
...
/* 	var tmpF = courbe; */
	...
	if (tmpArray.length > 4094) {
		npejies_error(this, "listdump aborted: too many points... use dump instead for now.");
		return;
	}
...
function MyName(courbe, name)
{
	if (typeof(name) == "string")
		...
		npejies_error(this, "function name must be a symbol");
		return;
	}
...
function SetColor(intOrcourbe, which, a, b, c)
{
	...
	if (tmpA.length == 3 && (typeof(tmpA[0]) == "number") && (typeof(tmpA[1]) == "number") && (typeof(tmpA[2]) == "number") )
		...
		npejies_error(this, "bad arguments for message", which);
}
...
function args4insp()
{
	...
	npejies_error(this, "since 1.52 the parameters are embed with the patcher. Use the inspector insteed.");
	return;
...
{
	if (v != 0 && v != 1) {
		npejies_error(this, "autosustain doesn't understand", v);
		return;
	}
	AutoSustain = v;
}
...
{
	if (v != 0 && v != 1)
		npejies_error(this, "autocursor doesn't understand", v);
	else
		CursorChange = v;
}
...
{
	if (arguments.length != 1) {
		npejies_error(this, "missing argument (name) for message addfunction");
		return;
	}
...
{
	if (arguments.length != 1) {
		npejies_error(this, "missing argument (name) for message insertfunction");
		return;
	}
...
function deletefunction()
{
	...
	if (which == undefined) {
		npejies_error(this, arguments[0], "bad function name (deletefunction aborted)");
		return;
	}
...
function setattr_bordersync(v)
{
	if (v == 0 || v == 1)
		...
		npejies_error(this, "bordersync doesn't understand", v);
}
...
function setattr_clickadd(v)
{
	if (v == 0 || v == 1)
		...
		npejies_error(this, "clickadd doesn't understand", v);
}
...
function setattr_clickmove(v)
{
	if (v == 0 || v == 1)
		...
		npejies_error(this, "clickmove doesn't understand", v);
}
...
function setattr_clicksustain(v)
{
	if (v == 0 || v == 1)
		...
		npejies_error(this, "clicksustain doesn't understand", v);
}
...
function display()
{
	...
	if ( arguments.length != 1 ) {
		npejies_error(this, "missing argument for message display");
		return;
	}
...
{
	if (arguments.length == 0)
		npejies_error(this, "missing arguments for messagae jit_matrix")
	else if (arguments.length == 1)
		setPointsFromMatrix(f[front], matrixName);
	else
		npejies_error(this,"extra arguments for messagae jit_matrix")
}
...
function clearsustain()
{
	if (! arguments.length)
		...
		npejies_error(this, "extra arguments for message clearsustain");
}
...
function fix()
{
	if (arguments.length == 2)
		...
		npejies_error(this, "bad arguments for message fix");
}

function unfix()
{
	if (arguments.length)
		npejies_error(this, "extra arguments for message unfix");
	else
		MyUnfix(f[front]);
}
...
function setattr_grid(v)
{
	if (v >= 0 && v <= 3) {
		...
		npejies_error(this, "gridmode doesn't understand", v);
}
...
function nth(v)
{
	if (typeof(v) == "number")
		...
		npejies_error(this, "bad argument for message nth");
}
...
function setattr_notifyrecalled(v)
{
	if (v == 0 || v == 1)
		...
		npejies_error(this, "notifyrecalled doesn't understand", v);
}
...
function active()
{
	...
	if (tmpArray.length > (NbCourbes + 1)) {
		npejies_error(this, "too many arguments for message active");
		return;
	}
	...
	if (tmpArray.length == 2) {
		if (tmpArray[1] == 0 || tmpArray[1] == 1) {
			...
			npejies_error(this, "bad argument for message active");
	}
...
function setattr_snap2grid(v)
{
	if (v >= 0 && v <= 3)
		...
		npejies_error(this, "snap2grid doesn't understand", v);
}
...
function setattr_hiddenpoint(v)
{
	if (v == 0 || v == 1) {
		...
		npejies_error(this, "hiddenpoint doesn't understand", v);
}
...
{
	if (v < 0 && v > 100)
		npejies_error(this, "ghost percentage between 0 and 100 % expected", v);
	else {
		Ghostness = v * 0.01;
		askForDrawFunctions();
	}
...
function setattr_timedisplay(v)
{
	if (v == 0 || v == 1)
		...
		npejies_error(this, "timedisplay doesn't understand", v);
}
...
function setattr_mousereport(v)
{
	if (v == 0 || v == 1)
		...
		npejies_error(this, "mousereport doesn't understand", v);
}
...
function next()
{
	if (! arguments.length)
		...
		npejies_error(this, "extra arguments for message next");
}
next.immediate = 1;
...
{
	if ( a >= b) {
		npejies_error(this, "bad values for message range: min must be lower than max");
		return ;
	}
...
{
	if ( a >= b) {
		npejies_error(this, "bad values for message setrange: min must be lower than max");
		return ;
	}
...
function copycolors()
{
	...
	if (c == -1) {
		npejies_error(this, arguments[0], "bad function name (copycolors aborted)");
		return;
	}
...
function pastecolors()
{
	if (! arguments.length)
		...
		npejies_error(this, "extra arguments for message pastecolors");
}
...
function zoom_x(start, stop, courbe)
{
	...
	if (arguments.length == 0) {
		...
		npejies_error(this, "missing argument for message zoom_x");
		return;
	}
...
function zoom_y(start, stop, courbe)
{
	...
	if (arguments.length == 0) {
		...
		npejies_error(this, "missing argument for message zoom_y");
		return;
	}
...
function onidle(x,y,but,cmd,shift,capslock,option,ctrl)
{
	...
	if (MouseReportState)
		outlet(DUMPOUT, "mouseidle", 	np_clip(x2val(f[front], x), f[front].domain[0], f[front].domain[1]),
										np_clip(y2val(f[front], y), f[front].range[0], f[front].range[1]),
										but, IdlePoint);
...
function onclick(x,y,but,cmd,shift,capslock,option,ctrl)
{
	...
	SelectedPoint = -2;
	x = np_clip(x - 2, Bordure, BoxWidth - Bordure);
	y = np_clip(y - 2, Bordure + LegendStateBordure, BoxHeight - Bordure);
	...
	if (MouseReportState) {
		if (isCurveMode) {
			outlet(DUMPOUT, "mouse",	np_clip(x2val(f[front], x), 
										f[front].domain[0], f[front].domain[1]),
										np_clip(y2val(f[front], y),
										f[front].range[0],
										f[front].range[1]),
										but,
										SelectedPoint);
...
function ondrag(x,y,but,cmd,shift,capslock,option,ctrl)
{
	...
	if (isCurveMode) {
		if(SelectedCurve >= 0) {
			...
			if(SelectedCurve >= 1) {
				...
				curr.curve = np_clip(curr.curve, CURVE_MIN, CURVE_MAX);
			...
			if (MouseReportState)
					outlet(DUMPOUT, "mouse", 	np_clip(x2val(f[front], x), f[front].domain[0], f[front].domain[1]),
												np_clip(y2val(f[front], y), f[front].range[0], f[front].range[1]),
	...
	/* 			drawText(); */
		...
		if (SelectedPoint < f[front].np) {
			...
			x = np_clip(x, Bordure, BoxWidth - Bordure);
			y = np_clip(y, Bordure + LegendStateBordure, BoxHeight - Bordure);
				...
				if (f[front]["np"] > 1) {
					...
					if (SelectedPoint == 0) {
						x = np_clip(x, 0, f[front]["pa"][SelectedPoint+1].x );  
					} else if (SelectedPoint > 0 && SelectedPoint < (f[front]["np"] - 1)) {
						x = np_clip(x, f[front]["pa"][SelectedPoint-1].x, f[front]["pa"][SelectedPoint+1].x);
					} else if (SelectedPoint == (f[front]["np"] - 1) ) {
						x = np_clip(x, f[front]["pa"][SelectedPoint-1].x, BoxWidth);
					}
				}
					...
					if(dx != 0) {
						for(i = SelectedPoint; i < f[front].np; i++) {
							if(MoveMode >= 4 && MoveMode <= 6)
								...
								f[front]["pa"][i].valx = np_clip(dx+f[front].pa[i].valx, f[front].domain[0], f[front].domain[1]);
...
function setvalueof()
{
	...
	if ( FunctionVersionCheck < 1 && FunctionVersionCheck > 6 ) {
		npejies_error(this, "bad version number - interpolation aborted");
		return;
	}
...
function copyfunction()
{
	...
	if (c == -1) {
		npejies_error(this, arguments[0], "bad function name (copyfunction aborted)");
		return;
	}
...
function pastefunction()
{
	if (! arguments.length)
		...
		npejies_error(this, "extra arguments for message pastefunction");
...
function MyPasteFunction(courbe)
{
	...
	if (cp.length == 0) {
		npejies_error(this, "before pasting, you have to copy something");
		return;
	}
...
function insertpaste()
{
	...
	if (cp.length == 0) {
		npejies_error(this, "before inserting and pasting, you have to copy something");
		return;
	}
...
function read(filename)
{
	...
	if (! arguments.length) {
		npejies_error(this, "read filename is missing");
		return;
	}
	...
	if (fichier.isopen) {
		...
		if (tmpLine.search("ej.function format") == -1) {
			npejies_error(this, "can't read this file format");
		...
		while ( fichier.position < fichier.eof && c < NbCourbes) {
			...
			for (p = 0; p < f[c].np; p++) {
				...
				if (! (tmpLine.length == 3 || tmpLine.length == 4)) {
					npejies_error(this, "bad file contents");
					break;
				}
		...
		npejies_error(this, "could not read file: ", filename, "... don't ask why :-)\n");
		outlet(DUMPOUT, "read", filename, -1);
	}
...
function write(filename)
{
	...
	if (arguments.length > 2) {
		npejies_error(this, "too many arguments for message write");
		return;
	}
	...
	if (arguments.length == 2) {
		if (arguments[1] == 1)
			...
			npejies_error(this, "bad arguments for message write");
	}
	...
	if (fichier.isopen) {
		...
		npejies_error(this, "could not write file: ", filename, "... don't ask why :-)\n");
		outlet(DUMPOUT, "write", filename, "-1");
	}
}
