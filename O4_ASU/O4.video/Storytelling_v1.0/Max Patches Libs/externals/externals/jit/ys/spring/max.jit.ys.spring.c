...
typedef struct _max_jit_ys_spring 
{
	t_object		ob;
	void			*obex;
...
void *max_jit_ys_spring_new(t_symbol *s, long argc, t_atom *argv);
void max_jit_ys_spring_mproc(t_max_jit_ys_spring *x, void *mop);
void max_jit_ys_spring_free(t_max_jit_ys_spring *x);
void max_jit_ys_spring_bang(t_max_jit_ys_spring *x);
void *max_jit_ys_spring_class;
...
{	
	...
	jit_ys_spring_init();	
	setup(&max_jit_ys_spring_class, max_jit_ys_spring_new, (method)max_jit_ys_spring_free, (short)sizeof(t_max_jit_ys_spring), 
		0L, A_GIMME, 0);

	p = max_jit_classex_setup(calcoffset(t_max_jit_ys_spring,obex));
	q = jit_class_findbyname(gensym("jit_ys_spring"));    
    	max_jit_classex_mop_wrap(p,q,MAX_JIT_MOP_FLAGS_OWN_BANG|MAX_JIT_MOP_FLAGS_OWN_OUTPUTMATRIX);
    	max_jit_classex_mop_mproc(p, q, max_jit_ys_spring_mproc); // custom mproc
	max_jit_classex_standard_wrap(p,q,0); 	

    	addmess((method)max_jit_mop_assist, "assist", A_CANT,0);
    	
    	// bang method (accept bang)
    	addbang((method)max_jit_ys_spring_bang);
...
void *max_jit_ys_spring_new(t_symbol *s, long argc, t_atom *argv)
{
	t_max_jit_ys_spring *x;
	void *o;

	if (x=(t_max_jit_ys_spring *)max_jit_obex_new(max_jit_ys_spring_class,gensym("jit_ys_spring"))) {
		if (o=jit_object_new(gensym("jit_ys_spring"))) {
