...
{	
	...
	jit_ys_timespace_init();	
	setup((t_messlist**)&max_jit_ys_timespace_class,
			(method)max_jit_ys_timespace_new,
			(method)max_jit_ys_timespace_free, (short)sizeof(t_max_jit_ys_timespace), 
		0L, A_GIMME, 0);

	p = max_jit_classex_setup(calcoffset(t_max_jit_ys_timespace,obex));
	q = jit_class_findbyname(gensym("jit_ys_timespace"));    
	max_jit_classex_mop_wrap(p,q,MAX_JIT_MOP_FLAGS_OWN_BANG|MAX_JIT_MOP_FLAGS_OWN_OUTPUTMATRIX);
	max_jit_classex_mop_mproc(p, q, max_jit_ys_timespace_mproc); // custom mproc
	max_jit_classex_standard_wrap(p,q,0); 	

	addmess((method)max_jit_mop_assist, "assist", A_CANT,0);
	
	// bang method (accept bang)
	addbang((method)max_jit_ys_timespace_bang);
