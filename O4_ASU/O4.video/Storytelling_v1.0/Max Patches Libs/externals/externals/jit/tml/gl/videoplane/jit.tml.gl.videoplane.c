...
typedef struct _jit_tml_gl_videoplane 
{
	t_object			ob;
	void				*ob3d;
	t_jit_glchunk		*chunk;
	...
	t_symbol			*colormode;
	SZ					*shaderCode;
} t_jit_tml_gl_videoplane;

void *_jit_tml_gl_videoplane_class;

t_jit_err jit_gl_videoplane_init(void) ;
t_jit_tml_gl_videoplane *jit_gl_videoplane_new(t_symbol * dest_name);
void jit_gl_videoplane_free(t_jit_tml_gl_videoplane *x);

t_jit_err jit_gl_videoplane_dim(t_jit_tml_gl_videoplane *x, void *attr, long argc, t_atom *argv);
t_jit_err jit_gl_videoplane_client_storage(t_jit_tml_gl_videoplane *x, void *attr, long argc, t_atom *argv);
t_jit_err jit_gl_videoplane_colormode(t_jit_tml_gl_videoplane *x, void *attr, long argc, t_atom *argv);
t_jit_err jit_gl_videoplane_interp(t_jit_tml_gl_videoplane *x, void *attr, long argc, t_atom *argv);
t_jit_err jit_gl_videoplane_rect_tex(t_jit_tml_gl_videoplane *x, void *attr, long argc, t_atom *argv);
t_jit_err jit_gl_videoplane_texturename(t_jit_tml_gl_videoplane *x, void *attr, long argc, t_atom *argv);
t_jit_err jit_gl_videoplane_displaylist(t_jit_tml_gl_videoplane *x, void *attr, long argc, t_atom *argv);
t_jit_err jit_gl_videoplane_draw(t_jit_tml_gl_videoplane *x);
t_jit_err jit_gl_videoplane_dest_closing(t_jit_tml_gl_videoplane *x);
t_jit_err jit_gl_videoplane_dest_changed(t_jit_tml_gl_videoplane *x);
t_jit_err jit_gl_videoplane_rebuild_geometry(t_jit_tml_gl_videoplane *x);
void jit_gl_videoplane_get_color(t_jit_tml_gl_videoplane *x, float *red, float *green, float *blue, float *alpha);
void jit_gl_videoplane_jit_matrix(t_jit_tml_gl_videoplane *x, t_symbol *s, int argc, t_atom *argv);
void jit_gl_videoplane_sendtexture(t_jit_tml_gl_videoplane *x, t_symbol *s, int argc, t_atom *argv);

t_jit_err jit_gl_videoplane_recalc(t_jit_tml_gl_videoplane *x);

void calc_plane(t_jit_tml_gl_videoplane *x);
void color_surface(t_jit_tml_gl_videoplane *x);
void draw_grid(t_jit_tml_gl_videoplane *x, t_jit_object *matrix, GLenum mode);
...
t_jit_err jit_gl_videoplane_init(void) 
{
	...
	_jit_tml_gl_videoplane_class = jit_class_new("jit_tml_gl_videoplane", (method)jit_gl_videoplane_new, (method)jit_gl_videoplane_free,
		sizeof(t_jit_tml_gl_videoplane),A_DEFSYM,0L);
	
	jit_class_addmethod(_jit_tml_gl_videoplane_class, (method)jit_gl_videoplane_jit_matrix,		"jit_matrix", 			A_USURP_LOW, 0);	
	jit_class_addmethod(_jit_tml_gl_videoplane_class, (method)jit_gl_videoplane_sendtexture,	"sendtexture", 			A_DEFER_LOW, 0);	

	// set up object extension for 3d object, customized with flags
	ob3d = jit_ob3d_setup(_jit_tml_gl_videoplane_class, calcoffset(t_jit_tml_gl_videoplane, ob3d), ob3d_flags);

	// add attributes
	attrflags = JIT_ATTR_GET_DEFER_LOW | JIT_ATTR_SET_USURP_LOW;
	attr = jit_object_new(_jit_sym_jit_attr_offset_array,"dim",_jit_sym_long,2,attrflags,
		(method)0L,(method)jit_gl_videoplane_dim,0/*fix*/,calcoffset(t_jit_tml_gl_videoplane,dim));
	jit_attr_addfilterset_clip(attr,2,0,TRUE,FALSE);
	jit_class_addattr(_jit_tml_gl_videoplane_class,attr);	
	attr = jit_object_new(_jit_sym_jit_attr_offset,"displaylist",_jit_sym_char,attrflags,
		(method)0L,(method)jit_gl_videoplane_displaylist,calcoffset(t_jit_tml_gl_videoplane, displaylist));	
	jit_class_addattr(_jit_tml_gl_videoplane_class,attr);	
	attr = jit_object_new(_jit_sym_jit_attr_offset,"gridmode",_jit_sym_char,attrflags,
		(method)0L,(method)0L,calcoffset(t_jit_tml_gl_videoplane, gridmode));	
	jit_class_addattr(_jit_tml_gl_videoplane_class,attr);	
	attr = jit_object_new(_jit_sym_jit_attr_offset,"interp",_jit_sym_long,attrflags,
		(method)0L,(method)jit_gl_videoplane_interp,calcoffset(t_jit_tml_gl_videoplane, interp));	
	jit_class_addattr(_jit_tml_gl_videoplane_class,attr);	
	attr = jit_object_new(_jit_sym_jit_attr_offset,"nudge",_jit_sym_float32,attrflags,
		(method)0L,(method)0L,calcoffset(t_jit_tml_gl_videoplane, nudge));	
	jit_class_addattr(_jit_tml_gl_videoplane_class,attr);
	attr = jit_object_new(_jit_sym_jit_attr_offset,"tex_offset_x",_jit_sym_float32,attrflags,
		(method)0L,(method)0L,calcoffset(t_jit_tml_gl_videoplane, tex_offset_x));	
	jit_class_addattr(_jit_tml_gl_videoplane_class,attr);
	attr = jit_object_new(_jit_sym_jit_attr_offset,"tex_offset_y",_jit_sym_float32,attrflags,
		(method)0L,(method)0L,calcoffset(t_jit_tml_gl_videoplane, tex_offset_y));	
	jit_class_addattr(_jit_tml_gl_videoplane_class,attr);
	attr = jit_object_new(_jit_sym_jit_attr_offset,"tex_scale_x",_jit_sym_float32,attrflags,
		(method)0L,(method)0L,calcoffset(t_jit_tml_gl_videoplane, tex_scale_x));	
	jit_class_addattr(_jit_tml_gl_videoplane_class,attr);
	attr = jit_object_new(_jit_sym_jit_attr_offset,"tex_scale_y",_jit_sym_float32,attrflags,
		(method)0L,(method)0L,calcoffset(t_jit_tml_gl_videoplane, tex_scale_y));	
	jit_class_addattr(_jit_tml_gl_videoplane_class,attr); 				
	attr = jit_object_new(_jit_sym_jit_attr_offset,"rect_tex",_jit_sym_long,attrflags,
		(method)0L,(method)jit_gl_videoplane_rect_tex,calcoffset(t_jit_tml_gl_videoplane, rect_tex));	
	jit_class_addattr(_jit_tml_gl_videoplane_class,attr);	
	attr = jit_object_new(_jit_sym_jit_attr_offset,"client_storage",_jit_sym_long,attrflags,
		(method)0L,(method)jit_gl_videoplane_client_storage,calcoffset(t_jit_tml_gl_videoplane, client_storage));	
	jit_class_addattr(_jit_tml_gl_videoplane_class,attr);	
	attr = jit_object_new(_jit_sym_jit_attr_offset,"colormode",_jit_sym_symbol,attrflags,
		(method)0L,(method)jit_gl_videoplane_colormode,calcoffset(t_jit_tml_gl_videoplane, colormode));	
	jit_class_addattr(_jit_tml_gl_videoplane_class,attr);	
	attr = jit_object_new(_jit_sym_jit_attr_offset,"texturename",_jit_sym_symbol,attrflags,
		(method)0L,(method)jit_gl_videoplane_texturename,calcoffset(t_jit_tml_gl_videoplane, texturename));	
	jit_class_addattr(_jit_tml_gl_videoplane_class,attr);	
	...
	jit_class_addmethod(_jit_tml_gl_videoplane_class, (method)jit_gl_videoplane_draw, "ob3d_draw", A_CANT, 0L);
	...
	// changed. 
	jit_class_addmethod(_jit_tml_gl_videoplane_class, (method)jit_gl_videoplane_dest_changed, "dest_changed",	A_CANT, 0L);
	jit_class_addmethod(_jit_tml_gl_videoplane_class, (method)jit_gl_videoplane_dest_closing, "dest_closing",	A_CANT, 0L);

	// rebuild_geometry is called if texture units or other 
	// attributes change requiring a rebuild
	jit_class_addmethod(_jit_tml_gl_videoplane_class, (method)jit_gl_videoplane_rebuild_geometry, "rebuild_geometry", 0L);
	
	// must register for ob3d	
	jit_class_addmethod(_jit_tml_gl_videoplane_class, (method)jit_object_register, 				"register",		A_CANT, 0L);

	jit_class_register(_jit_tml_gl_videoplane_class);
