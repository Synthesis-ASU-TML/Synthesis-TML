...
-------------------------------------------------------------------------*/
/*
Perform complex or hypercomplex signal correlation on control-rate data.
...
fft_complex_signal.prototype.correlate = function(signal_b)
{
  ...
  b.fft_decimation_in_frequency(true); 
...
/*-----------------------------------------------------------------------*/
fft_complex_signal.prototype.fft_decimation_in_time = function(is_simplex)
{ 
...
/*-----------------------------------------------------------------------*/
fft_complex_signal.prototype.fft_decimation_in_frequency = function(is_simplex)
{
...
function fft_quaternion_signal(N)
{
  ...
  this.phi   = this.simplex.theta;
  this.theta = this.perplex.rho;
  this.psi   = this.perplex.theta;
}

/*-----------------------------------------------------------------------*/
fft_quaternion_signal.prototype.slow_correlate = function(signal_b)
{
  var length = this.simplex.real.length;
  var result = new fft_quaternion_signal(2 * length);
  var a, b, c, d, e, f, g, h, index;
  ...
  for(i=-length; i<length; i++)
    {
      index = i + length;
      result.simplex.real[index] = 0;
      result.simplex.imag[index] = 0;
      result.perplex.real[index] = 0;
      result.perplex.imag[index] = 0;
      
      for(j=0; j<2*length; j++)
        {
          if((sig_a.simplex.real[i + j] != undefined) && (sig_b.simplex.real[j] != undefined))
            {
              a = sig_a.simplex.real[i + j] * 0.5;
              b = sig_a.simplex.imag[i + j] * 0.5;
              c = sig_a.perplex.real[i + j] * 0.5;
              d = sig_a.perplex.imag[i + j] * 0.5;
              e = sig_b.simplex.real[j] * 0.5;
              f = -sig_b.simplex.imag[j] * 0.5;
              g = -sig_b.perplex.real[j] * 0.5;
              h = -sig_b.perplex.imag[j] * 0.5;

              result.simplex.real[index] += (a*e) - (b*f) - (c*g) - (d*h);
              result.simplex.imag[index] += (a*f) + (b*e) + (c*h) - (d*g);
              result.perplex.real[index] += (a*g) - (b*h) + (c*e) + (d*f);
              result.perplex.imag[index] += (a*h) + (b*g) - (c*f) + (d*e);
          }
        }
    } 
...
/*-----------------------------------------------------------------------*/
fft_quaternion_signal.prototype.fast_correlate = function(signal_b)
{
  ...
  sig_b.perplex.bit_reverse_indices();
...
fft_quaternion_signal.prototype.fft_decimation_in_time = function()
{
  ...
  this.simplex.fft_decimation_in_time(true);
  this.perplex.fft_decimation_in_time(false);
}

/*-----------------------------------------------------------------------*/
fft_quaternion_signal.prototype.fft_decimation_in_frequency = function()
{
  //cf fft_decimation_in_time...
  this.simplex.fft_decimation_in_frequency(true);
  this.perplex.fft_decimation_in_frequency(false);
}
  ...
  for(i=0; i<this.rho.length; i++)
    {
      ...
      */
      this.phi[i]   = -Math.atan2 (2*(q2*q3 + q0*q1), -(q0*q0) + (q1*q1) + (q2*q2) - (q3*q3));
      this.theta[i] = -Math.asin  (2*(q0*q2 - q1*q3));
      this.psi[i]   =  Math.atan2 (2*(q1*q2 + q0*q3), (q0*q0) + (q1*q1) - (q2*q2) - (q3*q3));      
